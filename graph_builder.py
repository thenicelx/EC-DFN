...
class MultiScaleGraphBuilder:
    ...
    def _spatial_mask(self, radius_km):
        ...
        for i in range(self.n_grids):
            for j in range(self.n_grids):
                if i != j and geodesic(self.coords[i], self.coords[j]).km <= radius_km:
                    mask[i, j] = 1
        return mask

    def build_micro_graph(self, speed_features, dtw_window=5, theta=0.1):
        spatial_mask = self._spatial_mask(3)
    ...
    def build_macro_graph(self, adj_meso, alpha=0.85):
        out_degree = adj_meso.sum(1)
